...
miC <- modificationIndices(fit_1f_cov1, sort.=TRUE)

# Show the top candidates between observed indicators only:
miC_pairs <- miC[miC$op == "~~" & miC$lhs %in% smu7_names & miC$rhs %in% smu7_names, ]
...
#Twelve ITEMS: That is, including the active/passive thing
...
# Let's isolate the 1st and 2nd factor from the two-factor solution
...
# There are strong correlations (mostly > 0.9) for all the scales we may be deciding between.
# Therefore, 
# Let's stick to the most simple scale - isolate just one factor and do it just for the SMU-i and Specific Use Intensity: 
seldemog$fa_8_ml

# And put it into the data frame. And finally jdem samplovat.
...
# Both male and female data enter the same model -> indeed, we must standardise together...
# to tune down the scope of effects due to sex that are not related to iris colour itself. 
LAB$L_iris_scaled <- stan_L_iris(LAB$L_iris)
LAB$a_iris_scaled <- stan_a_iris(LAB$a_iris)
LAB$b_iris_scaled <- stan_b_iris(LAB$b_iris)

# But for future use (separate models), we also need within-sex scaling...
# ... and it's better to do it now than later. 

# Scale L* a* b* values (and do it per face):
# FEMALES
stan_L_iris_F <- function(x){(x-mean(LAB$L_iris[LAB$sex=="f"]))/sd(LAB$L_iris[LAB$sex=="f"])}
unstan_L_iris_F <- function(x){x*sd(LAB$L_iris[LAB$sex=="f"])+mean(LAB$L_iris[LAB$sex=="f"])}

stan_a_iris_F <- function(x){(x-mean(LAB$a_iris[LAB$sex=="f"]))/sd(LAB$a_iris[LAB$sex=="f"])}
unstan_a_iris_F <- function(x){x*sd(LAB$a_iris[LAB$sex=="f"])+mean(LAB$a_iris[LAB$sex=="f"])}

stan_b_iris_F <- function(x){(x-mean(LAB$b_iris[LAB$sex=="f"]))/sd(LAB$b_iris[LAB$sex=="f"])}
unstan_b_iris_F <- function(x){x*sd(LAB$b_iris[LAB$sex=="f"])+mean(LAB$b_iris[LAB$sex=="f"])}

# MALES
stan_L_iris_M <- function(x){(x-mean(LAB$L_iris[LAB$sex=="M"]))/sd(LAB$L_iris[LAB$sex=="M"])}
unstan_L_iris_M <- function(x){x*sd(LAB$L_iris[LAB$sex=="M"])+mean(LAB$L_iris[LAB$sex=="M"])}

stan_a_iris_M <- function(x){(x-mean(LAB$a_iris[LAB$sex=="M"]))/sd(LAB$a_iris[LAB$sex=="M"])}
unstan_a_iris_M <- function(x){x*sd(LAB$a_iris[LAB$sex=="M"])+mean(LAB$a_iris[LAB$sex=="M"])}

stan_b_iris_M <- function(x){(x-mean(LAB$b_iris[LAB$sex=="M"]))/sd(LAB$b_iris[LAB$sex=="M"])}
unstan_b_iris_M <- function(x){x*sd(LAB$b_iris[LAB$sex=="M"])+mean(LAB$b_iris[LAB$sex=="M"])}


LAB$L_iris_scaled_SexSep <- ifelse(LAB$sex=="f",stan_L_iris_F(LAB$L_iris),stan_L_iris_M(LAB$L_iris))
tapply(LAB$L_iris_scaled_SexSep,LAB$sex,mean) # Almost zero
tapply(LAB$L_iris_scaled,LAB$sex,mean) # Non-zero numbers... even though it is obvious the difference won't mean a thing

LAB$a_iris_scaled_SexSep <- ifelse(LAB$sex=="f",stan_L_iris_F(LAB$a_iris),stan_L_iris_M(LAB$a_iris))
LAB$b_iris_scaled_SexSep <- ifelse(LAB$sex=="f",stan_L_iris_F(LAB$b_iris),stan_L_iris_M(LAB$b_iris))
...
View(All_EYES_fin[,c(1:6,7,8:13,16:18,23,24,54:61)])

save(All_EYES_fin, file="All_EYES_fin_07_01_26.Rdata")
write.csv(All_EYES_fin, file="All_EYES_fin_07_01_26.csv",row.names=F)
